...
def lemmatizationWordnet(sentence):
    tokens = word_tokenize(sentence)
    tagged_sent = pos_tag(tokens)
    wnl = WordNetLemmatizer()
    lemmas_sent = []
...
title_content_wn_lemmatization = {}
for word, doc in tqdm.tqdm(wordnet_tagme_sememe_dict.items()):
    doc = lemmatizationWordnet(doc)
    title_content_wn_lemmatization[word] = doc

np.save("./data/title_content_lemmatization_sence", title_content_lemmatization)
np.save("./data/wordnet_lemmatization_sence", title_content_wn_lemmatization)
